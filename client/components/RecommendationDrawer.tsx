...
const RecommendationDrawer: React.FC<RecommendationDrawerProps> = ({ isOpen, onClose }) => {
  const products: Product[] = [
    {
      id: 'prev',
      name: 'Classic Sterling Band',
      price: '$19.99',
      image: 'https://api.builder.io/api/v1/image/assets/TEMP/79e899e6a64cc1d3cf3f16ddc85a962385344142?width=400',
      match: 'Partial Match',
      matchPercentage: 65
    },
    {
  ...
  const getMatchColor = (match: string) => {
    switch (match) {
      case 'Perfect Match': return 'text-green-400';
      case 'Decent Match': return 'text-grahun-yellow';
      case 'Partial Match': return 'text-orange-400';
  ...
  const getMatchBgColor = (match: string) => {
    switch (match) {
      case 'Perfect Match': return 'bg-green-400/20 border-green-400/30';
      case 'Decent Match': return 'bg-grahun-yellow/20 border-grahun-yellow/30';
      case 'Partial Match': return 'bg-orange-400/20 border-orange-400/30';
      default: return 'bg-grahun-white-20 border-grahun-white-20';
    }
  };
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-grahun-white-20">
          <div>
            <h3 className="text-white text-xl font-semibold">Recommendations</h3>
            <p className="text-grahun-white-40 text-sm">Based on your preferences</p>
          </div>
          <button 
            onClick={onClose}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <div className="text-grahun-white-40 text-xs">Match</div>
                        <div className={`font-semibold text-sm ${getMatchColor(product.match)}`}>
                    ...
                    {/* Match Badge */}
                    <div className={`inline-flex px-3 py-1 rounded-full text-xs font-medium border ${getMatchBgColor(product.match)} ${getMatchColor(product.match)}`}>
                      {product.match}
                    </div>
